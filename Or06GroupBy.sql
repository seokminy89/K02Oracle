...
group by
    ...
    -보여지는 건 하나의 레코드지만 여러개의 레코드가 묶여진 결과이므로
        통계적인 데이터를 계산할 수 있다.
...
select
    ...
    avg(salary) "평균급여2" ,
    trim(to_char(avg(salary), 'l999,000')) "평균급여3"
from employees;
...
/*
    ...
    문자열을 확인하는 것이 불가능하므로 일괄적인 규칙을 위해
    upper()와 같은 변환함수를 사용하여 검색하는것이 좋다.
*/
select 
    avg(salary),
    to_char(avg(salary), '$999,000.00') ,   
    ltrim(to_char(avg(salary), '$999,000.00')) "평균급여"
from employees
...
/*
    ...
    구할 수 있다.
    띠리사 문제에 따라 서브쿼리를 사용할 지 여부를 결정해야 한다.
*/
select * from employees where salary=(select min(salary) from employees);

/*
group by절 : 여러개의 레코드를 하나의 그룹으로 그룹화 하여 묶여진 결과를
    반환하는 쿼리문.
...
그 외의 단일 컬럼은select 절에서 사용할 수 없다.
...
/*
시나리오] 부서아이디가 50인 사원들의 직원 총합, 평균급여, 급여총합이 
...
시나리오] 사원테이블에서 각 부서별로 근무하고 있는 직원의 담당업무별 사원수와
    ...
    단, 사원의 총합이 10명을 초과하는 레코드만 출력하시오.
*/
select 
...
from employees;
/*
    위 커리문의 경우 그룹함수를 통한 결과를 출력하고 있으므로
...
select 
    job_id, 
    ltrim(to_char(max(salary), '999,000'))  MaxSalary, 
...
select
    department_id "부서번호", 
    count(*) "사원수", 
    rtrim(to_char(round(avg(salary), 2), '999,000.00')) "평균급여"
from employees
group by department_id
order by department_id asc;