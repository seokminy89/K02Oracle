...
--생성직후 cmd에서 sqlplus 명령으로 접속시 logon denied 에러 발생됨
...
테이블 스페이스란?
    ...
    hr 계정의 데이터를 저장하는 user 라는 테이블 스페이스가 자동으로 생성된다.
*/
--테이블 스페이스 조회하기.
...
--2번째 사용자 추가 : 테이블 스페이스 user를 사용할 수 있도록 생성한다.
create user test_user2 identified by 1234 default tablespace users;
grant create session to test_user2; --접속권한 부여
grant create table to test_user2; --테이블 생성권한 부여
--테이블 생성을 해보면 users 테이블스페이스에 대한 권한이 없어서 생성할 수 없음.

--test_user2가 사용하는 테이블 스페이스 확인.
select username, default_tablespace from dba_users
    where username in upper('test_user2'); --우리가 설정한 대로 users를 사용하고 있음
...
sqlplus에서 ED명령어 사용하기
    : 긴 쿼리문을 작성할 때 메모장을 editer로 활용하여 실행시키는 명령

1.파일생성하기
SQL> ed new_table [Enter]
    :이와 같은 명령으로 사용자계정 디렉토리에 new_table.sql 파일이 생성된다.

2.메모장에서 테이블 (혹은 다른 쿼리문)을 생성한다.
    메모장에서 작성할때 문장의 끝은 ; 이 아니라 / 를 붙여야 한다.
...
/*
4-1] 롤 생성하기 : 사용자가 원하는 권한을 묶어 새로운 룰을 생성한다.
*/
...
--cmd에서 접속 확인. 현재 접속은 가능하나, 테이블 스페이스가 없어 테이블 생성은 안됨.
...
/*
    teste_user3 사용자는 롤을 통해 권한을 부여받았으므로
