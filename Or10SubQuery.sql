...
/*
시나리오] 사원테이블에서 전체 사원의 평균급여보다 낮은 급여를 받는 사원들을
추출하여 출력하시오.
    출력하목 : 사원번호, 이름, 이메일, 연락처, 급여
*/
--해당 쿼리문은 문맥상 맞는듯 하지만 그룹함수를 단일행에 적용한 잘못된 쿼리
...
/*
복수행 연산자2 : any  자바의 or조건이랑 비슷함.
    메인쿼리의 비교조건이 서브쿼리의 검색결과와 하나 이상 일치하면
    참이 되는 연산자. 즉 둘중 하나만 만족하면 해당 레코드를 가져온다.
*/
...
select first_name, last_name, department_id, salary
    ...
    6000또는 13000 이상인 레코드만 추출된다.
    */

/*
복수행연산자3 : all  자바의 and 조건이랑 비슷.
...
select first_name, last_name, department_id, salary
    ...
    /*
        6000보다 크고, 13000보다도 커야 하므로 결과적으로 13000이상인 
        레코드만 추출하게 된다.
    */
...
Top쿼리 : 조회된 결과에서 구간을 정해 레코드를 가져올 때 사용한다.
    ...
    rownum : 테이블에서 레코드를 조회한 순서대로 순번이 부여되는
        가상의 컬럼을 말하며, 해당 컬럼은 모든 테이블에 존재한다.
*/
--모든 레코드를 가져온다.
select * from employees;
--정렬없이 레코드를 가져와서 rownum을 출력한다.(rownum이 순서대로 나옴)
select employee_id, first_name, rownum 
    from employees;
--이름으로 오름차순 정렬한 후 출력한다.(rownum이 섞여져서 나옴)
select employee_id, first_name, rownum 
    from employees order by first_name asc;
--rownum을 우리가 정렬한 순서대로 재부여하기 위해 서브쿼리를 활용한다.
...
#과제전용 > 02Oracle > 연습문제#(SubQuery) 폴더하위에 본인이름으로 업로드 합니다.
...
--오류발생. 그룹함수를 2개 겹쳤기 때문에 job컬럼을 제외해야 함.
...
/*
07.이름에 K가 포함된 사원과 같은 부서에서 일하는 사원의 사원번호와 이름을 표시하는 질의를 작성하시오
*/
...
*/
--평균급여
select avg(sal) from emp; --2077.xxxx
--K가 포함된 사원
