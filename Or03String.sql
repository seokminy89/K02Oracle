...
*/
select * from employees where first_name='john';--결과 없음(데이터는 대소문자 구분함)
...
select
    ...
    rtrim('우측문자열제거', '제거') rtrim1,
    rtrim('우측문자열제거 ', '문자열') rtrim2/*중간에 있는 문자열은 삭제안됨*/
from dual;
...
--trim은 좌우측의 공백은 제거되지만, 중간이 공백은 제거할 수 없다.
select trim(' 공백1 공백2 ') as blank1 from dual;
--replace는 좌우측 뿐만 아니라 중간의 공백도 제거할 수 있다.
select replace(' 공백1 공백2 ', ' ','') as blank2 from dual;

/*
사원테이블의 레코드를 대상으로 문자열 변경을 적용해 보자.
102	Lex	De Haan	LDEHAAN
*/
--102번 사원의 전체정보를 인출한다.
select * from employees where employee_id=102;
--위 함수를 통해 문자열 변경 몇가지를 해본다.
select 
