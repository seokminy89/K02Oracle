...
파일명 : Or02Number.sql
숫자(수학) 관련 함수
설명: 숫자데이터를 처리하기 위한 숫자관련 함수르를 알아보자
...
*/
--1.영업사원 찾기(영업사원의 job_id는 모두 SA_xxxx와 같이 시작한다.)
...
-- 첫번째 항목 : 소수이하 6자리까지 표현하므로 그 다음 7을 올림처리하여 0.123457이 됨
-- 두번째 항목 : 소수이하 4자리까지 표현하므로 그 다음 1을 버림처리하여 2.3456출력
select round(0.1234567, 6), round(2.345612, 4) from dual;

/*
시나리오] 사원 테이블에서 영업사원의 커미션을 반올림하여 출력하시오.
    단 소수점 1자리까지 표현하시오.
*/
