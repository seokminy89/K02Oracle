...
update tb_primary1 set idx=2 where user_name='코스모';/*
        update문은 정상이지만 idx값을 이미 존재하는 2로
...
alter table tb_primary3 add constraint tb_primary3_pk
    primary key (user_name); -- 제약명을 부여하면 오류 발생 시 해당 이름을 통해 확인할 수 있다.
desc tb_primary3;
select * from user_constraints;
...
--부모 테이블에 참조할 레코드가 없으므로 자식테이블에 입력불가
...
외래키 삭제시 옵션
[on delete cascade]
    : 부모 레코드 삭제시 자식 레코드까지 같이 삭제됨
    형식]
        컬럼명 자료형 references 부모테이블(pk컬럼)
            on delete cacade;
[on delete set null]
    : 부모 레코드 삭제시 자식 레코드 값이 null로 변경됨
    형식]
        컬럼명 자료형 references 부모테이블(pk컬럼)
            on delete set null;
...
insert into tb_foreign4 values (8, '난..어쯔??','gasan');--부모키가 없으므로 에러 발생

select * from tb_primary4;
select * from tb_foreign4;--삽입된 레코드가 있는것을 확인함.
...
참조키 부분만 null값으로 변경되어, 더이상 참조할 수 없는
레코드로 변경된다.
*/
...
create table tb_not_null (
    ...
    m_pw varchar2(20) null,       --null 허용(일반적으로 이렇게 쓰지는 않는다.)
    m_name varchar(50)            --null 허용(보통 이와 같이 선언한다.)
);
desc tb_not_null;
...
insert into tb_not_null (m_id,m_pw, m_name) 
    values ('hong5','5555','오길동'); --오류발생 PK에는 null 입력할 수 없음.
...
/*
    위에서 보듯이 default값을 입력하려면 insert시 컬럼자체를
...
insert into tb_check1 values('A'); --입력 실패
insert into tb_check1 values('여성');--입력 실패
...
/*
#과제전용 > 02Oracle > 연습문제#(Constraint) 폴더하위에 본인이름으로 업로드 합니다.
...
alter table pr_emp_const /* 외래키는 자식테이블에서 생성한다.*/
    add constraint pr_emp_dept_fk /* 제약 조건명 추가*/
    foreign key (deptno) /* 자식테이블에서 외래키(참조키) 컬럼 지정*/
    references pr_dept_const (deptno); /* 부모테이블의 기본키 컬럼 지정*/
...
insert into pr_emp_const values
    (100, '아무개', '열정', null, sysdate, 1000, 0.5, 10); --입력실패(현재 부모키 없음)
insert into pr_emp_const values
    (200, '머시기', '냉정', null, sysdate, 2000, 0, 10);-- 입력실패(check 제약조건 위배)
...
delete from pr_dept_const; -- 자식 레코드가 있으므로 삭제 불가. 외래키 제약조건 위배.

--외래키 제약조건을 재설정 하기 위해 기존의 제약조건 삭제
alter table pr_emp_const drop constraint pr_emp_dept_fk;
-- 외래키 재설정 : 부모 레코드 삭제시 자식 레코드까지 동시에 삭제되도록 설정
alter table pr_emp_const
...
delete from pr_dept_const; --자식 레코드까지 모두 삭제됨
