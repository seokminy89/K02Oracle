...
형변환 함수 / 기타함수
설명: 데이터 타입을 다른 타입으로 변환해야 할 때 사용하는 함수와 기타함수.
**********************/

/*
sysdate : 현재 날짜와 시간을 초단위로 반환해 준다.
주로 게시판에서 새로운 포스팅이 있을때 입력한 날짜를 표현하기 위해 사용된다.
*/
...
/* 시간 포맷 */
--현재 시간을 00:00:00 형태로 표기(대소문자 구분없음)
select
...
숫자포맷
    0 : 숫자의 자리수를 나타내며 자리수가 맞지 않는 경우
        0으로 자리를 채운다.
    0 : 0과 동일하지만, 자리수가 맞지 않는 경우 공백으로 채운다.
*/
...
from dual;--오라클에서는 함수를 2개이상 중첩해서 사용할 수 있다.(ex 0,9)
...
/*
시나리오]'2021-10-14 15:51:39' 와 같은 형태의 문자열을 날짜로 인식할 수 있도록
    쿼리문을 작성하시오.
*/
...
select
    ...
    to_date(substr('2021-10-14 15:51:39', 1, 10)) "날짜서식으로 변경"
from dual;
...
select 
    to_date('2021/05/05'),
    to_char(to_date('2021/05/05'), 'day"입니다."')
from dual;
*/
...
/*
시나리오] 사원테이블의 사원의 입사일을 요일까지 표현하시오.
*/
select 
...
nvl() : null값을 다른 데이터로 변경하는 함수.
    ...
    ※레코드를 selelct해서 웹브라우저에 출력을 하는 경우 해당 컬럼이 null이면
    NullPointerException이 발생하게 된다. 그러므로 아예 데이터를 가져올 때
    null값이 나올 수 있는 컬럼에 대해 미리 처리하면 예외 발생을 차단할 수 
    있으므로 편리하다.
*/
select
    first_name, commission_pct, nvl(commission_pct, 0) as "보너스율",
    (salary+commission_pct) "결과비정상",(salary+nvl(commission_pct, 0)) "결과정상"
from employees;

/*
decode() : java의 switch문과 비슷하게 특정값에 해당하는 출력문이 있는 경우
    사용한다.
    형식] decode(컬럼명,
                값1, 결과1, 값2, 결과2, .....
                기본값)
...
--아래는 내가 한것.
select ename from emp where substr(hiredate, 5, 1)=4;
...
select substr(hiredate, 1, 2)YY,substr(hiredate, 4, 2)MON,substr(hiredate,7,2)dy from emp;
...
from dual;
-- 아래 내가 한것.
select to_date(sysdate)-to_date('01-01-2021','dd-mm-yyyy') from dual;
/*
...
select ename 이름, sal 급여, decode(job, 'clerk',sal+200,'salesman',sal+180,
    'manager',sal+150,'presdent',sal+100 )인상된급여 from emp;
