...
execute pcd_in_param_salary('Nancy');
...
begin 
    ...
    */
    ...
    /*
    행의 변화가 있는 쿼리를 실행할경우 반드시 commit 해야 실제테이블에
...
create or replace function calSumBetween (
    num1 in number,
    num2 number --함수는 in파라미터만 사용가능하므로 생략할수있다.
)
return 
...
--실행방법2 : 바인드변수를 통한 실행명령으로 사용한다.(디버깅용)
...
create or replace trigger trig_dept_backup
	...
	for each row /*
        ...
        해당 문장을 제거하면된다. 이때에는 쿼리를 한번 실행할때 
        트리거도 한번만 실행된다. 
    */
begin
...
execute KosmoMemberInsert('pro02', '1234', '프로시저1', :i_result);
print i_result; 
select * from member;
...
/
--둘다일치, 아이디만일치, 둘다틀린 3가지 케이스를 모두 테스트 해볼것
variable member_auth number;
execute KosmoMemberAuth('test9999','12345',:member_auth);
print member_auth;
