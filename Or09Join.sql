...
outer join을 사용할 때는 outer전에 기준이 되는 테이블을 결정하고
기술해야 한다.
...
--위 시나리오를 테이블의 기준 위치만 right로 변경해서 실행해보자
select
    employee_id, first_name, last_name, D.department_id, city
from employees E
...
*/
/*
시나리오] 사원테이블에서 각 사원의 매니져 아이디와 매니져 이름을 출력하시오.
    단, 이름은 firts_name과 last_name을 하나로 연결해서 출력하시오.
*/
select
...
--2.위에 서 알게된 킴벌리의 입사일 07/05/24 이후에 입사한 직원들을 인출
...
/*
    ...
    붙이면 오류가 발생한다.
    using절에 사용된 컬럼은 좌, 우측의 테이블에 동시에 존재하는 컬럼이라는 것을
    전제로 작성되기 때문에 굳이 별칭을 붙여줄 이유가 없기 때문이다.
*/
...
--아래 내가 한거
--1. 2005년에 입사한 사원들 출력
select first_name from employees where to_char(hire_date,'yyyy')='2005';
...
where
    to_char(hire_date,'yyyy')='2005'
    and state_province = 'California' and city='South San Francisco'; 

    
    

----------------------------------------------------------------
--연습문제(hr 계정 employees 사용)
/*
