...
SELECT employee_id "사원번호", first_name, email
    FROM employees WHERE first_name='ALEXander';--이름에 대소문자가 잘못 입력됨.
...
/*
비교연산자를 통한 쿼리작성
    : 이상, 이하와 같은 조건에 >, < = 같은 비교연산자를 사용할 수 있다.
...
/*
in 연산자 :or 연산자와 비슷한 기능으로 하나의 컬럼에 여러 값의 조건을 
    걸고 싶을 때 사용하는 연산자.
...
distinct 연산자 : 컬럼에서 중복되는 레코드를 제거할 때 사용한다.
    특정 조건으로 select 했을때 하나의 컬럼에서 중복되는 값이 
...
like 연산자 : 특정 키워드를 통한 문자열 검색하기.
    ...
    와일드카드 사용법
        ...
        Ex) D로 시작하는 단어 : D% -> Da, Dae, Daewoo
            Z로 끝나는 단어 : %Z -> aZ, abcZ
            C가 포함되는 단어 : %C% -> aCb, abCde, Vitanim-C
        _ : 언더바는 하나의 문자를 대체한다.
...
*/
/*
사원정보 테이블에서 급여가 낮은순서에서 높은 순서로 나오도록 정렬하여 조회하시오.
...
select ename"이름", sal"급여" from emp where sal>2000 order by sal desc;
...
select ename, sal, deptno from emp where sal between 2000 and 3000 
    and deptno in(20,30) order by ename asc;
/*
...
--밑은 내가 한거.
/*select ename, sal, comm from emp 
    where comm!=0 and comm is not null order by sal desc , comm desc;*/

/*
12. 이름의 세번째 문자가 R인 사원의 이름을 표시하시오.
*/
select ename from emp where ename like '__R%'; -- 만약 끝에 %가 없으면 이름이 총 세글자인 사람만 검색한다.
/*
