...
/*
시나리오] hr 계정의 사원테이블에서 담당업무가 ST_CLERK인 사원의 정보를
...
--밑은 내가 하려고 한거 실행안됨.
select employee_id, concat(first_namelast_name), email, manager_id
    from employees
    where job_id='ST_MAN';
...
select employee_id, first_name, 
    ...
    from employees;/*
    보너스율이 null인 경우 연봉이 계산되지 않으므로 nvl()함수를
    이용해서 0으로 값을 변환한 후 계산한다. 또한 to char()로
