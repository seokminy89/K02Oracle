...
/*
시나리오] employees 테이블에 입력된 직원들의 근속개월수를 계산하여 출력하시오.
결과를 근속개월수로 오름차순 정렬하시오.
*/
select 
...
select last_day('20/02/01') from dual; --20년은 윤년이므로 29일로 출력됨

--컬럼이 date 형인 경우 간단한 날짜연산이 가능하다.
select
    sysdate "현재날짜",
    sysdate+1 "내일날짜",
    sysdate-1 "어제날짜",
