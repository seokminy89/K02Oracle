...
시퀀스 & 인덱스
설명 : 테이블의 기본키 필드에 순차적인 일렬번호를 부여하는 시퀀스와
    검색속도를 향상시킬 수 있는 인덱스
******************/
...
/*
시퀀스 
-테이블의 컬럼(필드)에 중복되지 않는 순차적인 일렬번호를 부여한다.
...
1. start with에 minvalue보다 작은 값을 지정할 수 없다. 즉 start with값은
    minvalue와 같거나 커야한다.
2. nocycle로 설정하고 시퀀스를 계속 얻어올 때 maxvalue의 지정값을 초과하면
    에러가 발생한다.
...
/*
    시퀀스 생성 후 최초 실행시에는 오류가 발생된다. nextval을 먼저
...
/*
다음 입력할 일련번호(시퀀스)를 반환한다. 실행할 때마다 다ㄴ음으로 넘어간다.
*/
...
insert into tb_goods values (seq_serial_num.nextval, '초콜렛13');/*
            시퀀스의 cycle옵션에 의해 최대ㅔ값에 도달하면 다시 처음부터
            일렬번호가 생성되므로 무결성 제약조건에 위배된다. 즉 기본키에
...
start with는 수정할 수 없다. 이미 생성된 시퀀스의 초기값은 수정할 수 없다.
*/
...
create sequence seq_serial_num
    ...
    nocache;
/*
    일반적으로 stat with, increment, minvalue는 1로 설정하고
    나머지는 no로 설정한다.
*/
select * from tb_goods;
...
-인덱스는 명시적(create index) 와 자동적(primary key, unique)
    으로 생성할 수 있다.
...
--인덱스 수정: 인덱스는 수정이 불가능하다. 따라서 삭제 후 다시 생성해야 한다.
