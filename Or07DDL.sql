...
새로운 계정을 생서한 후 접속권한과 테이블 생성권한 몇가지를 부여한다.
*/
create user study identified by 1234; --study 계정 생성.
grant connect, resource to study; --생성한 계정에 권한을 부여.
...
기존 생성된 테이블에 새로운 컬럼 추가하기
    -> tb_memeber 테이블에 email 컬럼을 추가하시오.
...
/*
기존 생성된 테이블의 컬럼 수정하기
    -> tb_member 테이블의 email 커럼의 사이즈를 200으로 확장하시오.
